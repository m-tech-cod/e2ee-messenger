...
export async function generateRSAKeyPair() {
  ...
  return {
    ...
    privateKey: bufferToBase64(await crypto.subtle.exportKey("pkcs8", pair.privateKey))
  };
}
...
export async function decryptMessage(payload, privateKeyBase64, isOwnMessage = false) {
  ...
  const plain = await crypto.subtle.decrypt({ name: "AES-GCM", iv: base64ToBuffer(payload.iv) }, aesKey, base64ToBuffer(payload.ciphertext));
